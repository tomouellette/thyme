...
/// Available neural network models in the pineapple library.
pub enum Weights {
    DinoVitSmall,
    DinoVitBase,
    DinobloomVitBase,
...
impl Weights {
    ...
    pub fn select(weights_name: &str) -> Self {
        match weights_name {
            ...
            _ => {
                let msg = format!(
                    "[pineapple::data::weights] Weights {} not found. Avalaible weights include: {}",
                    weights_name,
    ...
    /// Download the model to the pineapple cache.
    pub fn download(&self, verbose: bool) {
        ...
        if !model_name.exists() {
            ...
            if !model_name.exists() {
                eprintln!("[pineapple::data::weights] Failed to download model weights.");
                std::process::exit(1);
            }
        }
    }
