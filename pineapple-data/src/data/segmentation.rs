...
/// Available annotated/segmentation datasets in the pineapple library.
...
impl SegmentationDatasets {
    ...
    pub fn select(name: &str) -> Self {
        match name {
            ...
            _ => {
                let msg = format!(
                    "[pineapple::data::segmentation] Dataset {} not found. Avalaible datasets include: {}",
                    name,
    ...
    pub fn download(&self, output: &Path, verbose: bool) {
        ...
        if !output.join(&filename).exists() {
            ...
            if !output.join(&filename).exists() {
                eprintln!("[pineapple::data::segmentation] Failed to download segmentation dataset.");
                std::process::exit(1);
            }
        }
    }
