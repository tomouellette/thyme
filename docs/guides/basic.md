...
`pineapple process` takes in paired image and segmentation data and extracts a vareity of object-level
data including crops, polygons, masks, and bounding boxes.
...
`pineapple profile` takes in paired image and segmentation data and computes a variety of morphological
descriptors (e.g. area, eccentricity, moments, etc.) for every segmented object in each image.
Descriptors can be saved in `csv`, `txt`, `tsv`, or `pq` (parquet) formats.
...
`pineapple neural` takes in paired image and segmentation data and computes self-supervised features/embeddings
for every segmented object in each image.
Self-supervised features can be saved in `csv`, `txt`, `tsv`, `pq` (parquet), `npy`, or `npz` formats.
...
If you want to compute quantitative features directly from images or polygons without associated
segmentation data, then you can use `pineapple measure`.
Various quantitative features can be computed and saved as follows.
...
Note that `images2zarrs` encodes image name strings as fixed-width numpy-style arrays (max length of 100).
We currently do this as current zarr string decoding is inconsistent across different implementations.
If you are loading the data in python, the saved image names can be mapped to strings via utf8 decoding as follows.
...
!!! note
    Data is currently stored on Google Drive so it's possible downloads may be throttled.
    A Zenodo backup is planned to allow a slower, more reliable download in cases where 
    Google Drive fails.
