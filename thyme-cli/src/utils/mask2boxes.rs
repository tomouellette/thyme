...
pub fn utils_mask2boxes(args: &Mask2boxesArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[thyme::utils::mask2boxes] ERROR: Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    let is_mask_dir = if let Some(ext) = mask_extension {
        if !constant::SUPPORTED_IMAGE_FORMATS.contains(&ext.as_str()) {
            eprintln!(
                "[thyme::utils::mask2boxes] ERROR: Invalid mask extension {}. Must be one of: {:?}.",
                ext,
    ...
    if let Some(ext) = output_extension {
        if is_mask_dir {
            eprintln!(
                "[thyme::utils::mask2boxes] ERROR: If mask input is a directory then output must be a directory."
            );
            std::process::exit(1);
        }
        ...
        if !["json"].iter().any(|e| e == &ext) {
            eprintln!(
                "[thyme::utils::mask2boxes] ERROR: Invalid file extension. Must end with .json."
            );
            std::process::exit(1);
        }
        ...
        mask2boxes(Path::new(&mask_path), &output, false).unwrap_or_else(|_| {
            eprintln!(
                "[thyme::utils::mask2boxes] ERROR: Failed to convert mask to bounding boxes."
            );
            std::process::exit(1);
        });
    } else {
        if !is_mask_dir {
            eprintln!(
                "[thyme::utils::mask2boxes] ERROR: If output is a directory then mask input must be a directory."
            );
            std::process::exit(1);
        }
        ...
        if mask_files.is_empty() {
            eprintln!(
                "[thyme::utils::mask2boxes] ERROR: No mask files were detected. Please check your path and/or substring identifier."
            );
            std::process::exit(1);
        }
        ...
        output = ut::path::create_directory(&output).unwrap_or_else(|_| {
            eprintln!("[thyme::utils::mask2boxes] ERROR: Could not create directory.");
            std::process::exit(1);
        });
...
fn mask2boxes(mask_path: &Path, output_path: &Path, is_dir: bool) -> Result<(), ThymeError> {
