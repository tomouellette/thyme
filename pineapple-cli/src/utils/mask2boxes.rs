...
pub fn utils_mask2boxes(args: &Mask2boxesArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::utils::mask2boxes] ERROR: Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    let is_mask_dir = if let Some(ext) = mask_extension {
        if !constant::SUPPORTED_IMAGE_FORMATS.contains(&ext.as_str()) {
            eprintln!(
                "[pineapple::utils::mask2boxes] ERROR: Invalid mask extension {}. Must be one of: {:?}.",
                ext,
    ...
    if let Some(ext) = output_extension {
        if is_mask_dir {
            eprintln!(
                "[pineapple::utils::mask2boxes] ERROR: If mask input is a directory then output must be a directory."
            );
            std::process::exit(1);
        }
        ...
        if !["json"].iter().any(|e| e == &ext) {
            eprintln!(
                "[pineapple::utils::mask2boxes] ERROR: Invalid file extension. Must end with .json."
            );
            std::process::exit(1);
        }
        ...
        mask2boxes(Path::new(&mask_path), &output, false).unwrap_or_else(|_| {
            eprintln!(
                "[pineapple::utils::mask2boxes] ERROR: Failed to convert mask to bounding boxes."
            );
            std::process::exit(1);
        });
    } else {
        if !is_mask_dir {
            eprintln!(
                "[pineapple::utils::mask2boxes] ERROR: If output is a directory then mask input must be a directory."
            );
            std::process::exit(1);
        }
        ...
        if mask_files.is_empty() {
            eprintln!(
                "[pineapple::utils::mask2boxes] ERROR: No mask files were detected. Please check your path and/or substring identifier."
            );
            std::process::exit(1);
        }
        ...
        output = ut::path::create_directory(&output).unwrap_or_else(|_| {
            eprintln!("[pineapple::utils::mask2boxes] ERROR: Could not create directory.");
            std::process::exit(1);
        });
        ...
        (0..mask_files.len())
            ...
            .tqdm_with_bar(pb)
            .for_each(|idx| {
                mask2boxes(&mask_files[idx], &output, true).unwrap_or_else(|_| {
        ...
        let message = if !error.is_empty() {
            &format!(
                "Complete. {} images succesfully converted to bounding boxes. {} images failed.",
                ut::track::thousands_format(mask_files.len() - error.len()),
                ut::track::thousands_format(error.len())
            )
        } else {
            &format!(
                "Complete. {} images converted to bounding boxes.",
                ut::track::thousands_format(mask_files.len() - error.len()),
            )
        };
...
/// Convert an input mask to bounding boxes
fn mask2boxes(mask_path: &Path, output_path: &Path, is_dir: bool) -> Result<(), PineappleError> {
    ...
    if is_dir {
        ...
        bounding_boxes.save(output_path)?;
    }
