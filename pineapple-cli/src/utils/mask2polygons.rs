...
pub fn utils_mask2polygons(args: &Mask2polygonsArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::utils::mask2polygons] ERROR: Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    let is_mask_dir = if let Some(ext) = mask_extension {
        if !constant::SUPPORTED_IMAGE_FORMATS.contains(&ext.as_str()) {
            eprintln!(
                "[pineapple::utils::mask2polygons] ERROR: Invalid mask extension {}. Must be one of: {:?}.",
                ext,
    ...
    if let Some(ext) = output_extension {
        if is_mask_dir {
            eprintln!(
                "[pineapple::utils::mask2polygons] ERROR: If mask input is a directory then output must be a directory."
            );
            std::process::exit(1);
        }
        ...
        if !["json"].iter().any(|e| e == &ext) {
            eprintln!(
                "[pineapple::utils::mask2polygons] ERROR: Invalid file extension. Must end with .json."
            );
            std::process::exit(1);
        }
        ...
        if let Some(parent) = output.parent() {
            if !parent.is_dir() && parent.to_str().unwrap() != "" {
                eprintln!(
                    "[pineapple::utils::mask2polygons] ERROR: Invalid file path. Parent directory of output file path does not exist."
                );
                std::process::exit(1);
            }
        }
        ...
        mask2polygons(Path::new(&mask_path), &output, false).unwrap_or_else(|_| {
            eprintln!("[pineapple::utils::mask2polygons] ERROR: Failed to convert mask to polygons.");
            std::process::exit(1);
        });
    } else {
        if !is_mask_dir {
            eprintln!(
                "[pineapple::utils::mask2polygons] ERROR: If output is a directory then mask input must be a directory."
            );
            std::process::exit(1);
        }
        ...
        if mask_files.is_empty() {
            eprintln!(
                "[pineapple::utils::mask2polygons] ERROR: No mask files were detected. Please check your path and/or substring identifier."
            );
            std::process::exit(1);
        }
        ...
        output = ut::path::create_directory(&output).unwrap_or_else(|_| {
            eprintln!("[pineapple::utils::mask2polygons] ERROR: Could not create directory.");
            std::process::exit(1);
        });
...
fn mask2polygons(mask_path: &Path, output_path: &Path, is_dir: bool) -> Result<(), PineappleError> {
