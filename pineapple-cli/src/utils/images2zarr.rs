...
pub fn utils_images2zarr(args: &Images2zarrArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::utils::images2zarr] ERROR: Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    let (zarr_dtype, zarr_fill) = match dtype.as_str() {
        ...
        _ => {
            eprintln!(
                "[pineapple::utils::images2zarr] ERROR: Invalid dtype. Only u8, u16, f32, f64 data types are supported."
            );
            std::process::exit(1);
        }
    };
    ...
    if !(0..=9).contains(&gzip_compression) {
        eprintln!(
            "[pineapple::utils::images2zarr] ERROR: Invalid gzip_compression. Must be 0 to 9 inclusive."
        );
        std::process::exit(1);
    }
    ...
    if image_files.is_empty() {
        eprintln!(
            "[pineapple::utils::images2zarr] ERROR: No image files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if let Some(ext) = extension {
        if "zarr" != ext {
            eprintln!(
                "[pineapple::utils::images2zarr] ERROR: Invalid output extension. Must end with one of zarr."
            );
            std::process::exit(1);
        }
    ...
    let store: ReadableWritableListableStorage =
        Arc::new(FilesystemStore::new(output).unwrap_or_else(|_| {
            eprintln!("[pineapple::utils::images2zarr] Failed to create zarr filesystem store.");
            std::process::exit(1);
        }));
    ...
    let images_builder = if gzip_compression == 0 {
        ...
        eprintln!("[pineapple::utils::images2zarr] Gzip compression level must be in [0, 9].");
        std::process::exit(1);
    };
    ...
    (0..image_files.len())
        ...
        .tqdm_with_bar(pb)
        .for_each(|idx| {
            let image = im::PineappleImage::open(&image_files[idx]);
