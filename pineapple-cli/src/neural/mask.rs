...
pub fn neural_image_mask(args: &NeuralMaskArgs) {
    let device = args.device.to_owned().unwrap_or("cpu".to_string());

    if !["cpu", "metal", "cuda"].iter().any(|d| d == &device) {
        eprintln!("[pineapple::neural::mask] ERROR: Invalid device. Must be one of: cpu, metal, cuda.");
        std::process::exit(1);
    }

    if device == "cuda" && !cuda_is_available() {
        println!("[pineapple::neural::mask] Device 'cuda' specified but no cuda device was detected.");
        std::process::exit(1);
    }

    if device == "metal" && !metal_is_available() {
        println!(
            "[pineapple::neural::mask] Device 'metal' specified but no metal device was detected."
        );
        std::process::exit(1);
    }
    ...
    if threads < 1 {
        println!("[pineapple::neural::mask] Threads must be set to a positive integer if provided.");
        std::process::exit(1);
    }
    ...
    if min_size < 1 {
        eprintln!("[pineapple::neural::mask] ERROR: min_size cannot be less than 1.0.");
        std::process::exit(1);
    }
    ...
    if image_path == masks_path && args.image_substring == args.mask_substring {
        eprintln!(
            "[pineapple::neural::mask] ERROR: If images and masks are located in same path, different image and mask substrings must be provided."
        );
        std::process::exit(1);
    }
    ...
    if image_files.is_empty() {
        eprintln!(
            "[pineapple::neural::mask] ERROR: No image files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if mask_files.is_empty() {
        eprintln!(
            "[pineapple::neural::mask] ERROR: No mask files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if let Some(ext) = &extension {
        ...
        {
            eprintln!(
                "[pineapple::neural::mask] ERROR: Invalid file extension. Must end with one of .npy, .npz, .csv, .txt, .tsv, .pq."
            );
            std::process::exit(1);
        }
        ...
        output = ut::path::create_directory(&output).unwrap_or_else(|_| {
            eprintln!("[pineapple::neural::mask] ERROR: Could not create directory.");
            std::process::exit(1);
        });
    }
        ...
        if let Some(ext) = &extension {
            if ["csv", "txt", "tsv", "pq"].iter().any(|e| e == ext) {
                ...
                io::write_table(&mut df, &output).unwrap_or_else(|_| {
                    eprintln!(
                        "[pineapple::neural::mask] ERROR: Failed to write embeddings to a table."
                    );
                    std::process::exit(1);
                });
            } else if ext == "npy" {
                ...
                .unwrap_or_else(|_| {
                    eprintln!(
                        "[pineapple::neural::mask] ERROR: Failed to write embeddings to a npy array."
                    );
                    std::process::exit(1);
                });
            } else if ext == "npz" {
                io::write_embeddings_npz(name, item, spot, data, &output).unwrap_or_else(|_| {
                    eprintln!(
                        "[pineapple::neural::mask] ERROR: Failed to write embeddings to an npz array."
                    );
                    std::process::exit(1);
                });
            }
...
fn neural(
    ...
    model: &Models,
    device: &Device,
) -> Result<(Vec<u32>, Vec<[f32; 2]>, Vec<Vec<f32>>), PineappleError> {
    let image = im::PineappleImage::open(image_path)?;

    let mut mask = im::PineappleMask::open(mask_path)?;

    if image.width() != mask.width() || image.height() != mask.height() {
        return Err(PineappleError::OtherError(
