...
pub fn neural_image_boxes(args: &NeuralBoxesArgs) {
    ...
    if !["cpu", "metal", "cuda"].iter().any(|d| d == &device) {
        eprintln!(
            "[pineapple::neural::boxes] ERROR: Invalid device. Must be one of: cpu, metal, cuda."
        );
        std::process::exit(1);
    }

    if device == "cuda" && !cuda_is_available() {
        println!("[pineapple::neural::boxes] Device 'cuda' specified but no cuda device was detected.");
        std::process::exit(1);
    }

    if device == "metal" && !metal_is_available() {
        println!(
            "[pineapple::neural::boxes] Device 'metal' specified but no metal device was detected."
        );
        std::process::exit(1);
    }
    ...
    if threads < 1 {
        println!("[pineapple::neural::boxes] Threads must be set to a positive integer if provided.");
        std::process::exit(1);
    }
    ...
    if min_size < 1 {
        eprintln!("[pineapple::neural::boxes] ERROR: min_size cannot be less than 1.0.");
        std::process::exit(1);
    }
    ...
    if image_path == boxes_path && args.image_substring == args.box_substring {
        eprintln!(
            "[pineapple::neural::boxes] ERROR: If images and boxes are located in same path, different image and bounding box substrings must be provided."
        );
        std::process::exit(1);
    }
    ...
    if image_files.is_empty() {
        eprintln!(
            "[pineapple::neural::boxes] ERROR: No image files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if boxes_files.is_empty() {
        eprintln!(
            "[pineapple::neural::boxes] ERROR: No bounding box files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if let Some(ext) = &extension {
        ...
        {
            eprintln!(
                "[pineapple::neural::boxes] ERROR: Invalid file extension. Must end with one of .npy, .npz, .csv, .txt, .tsv, .pq."
            );
            std::process::exit(1);
        }
        ...
        if let Some(parent) = output.parent()
            ...
            eprintln!(
                "[pineapple::neural::boxes] ERROR: Invalid file path. Parent directory of output file path does not exist."
            );
            std::process::exit(1);
        }
    } else {
        output = ut::path::create_directory(&output).unwrap_or_else(|_| {
            eprintln!("[pineapple::neural::boxes] ERROR: Could not create directory.");
            std::process::exit(1);
        });
    }
        ...
        if let Some(ext) = &extension {
            if ["csv", "txt", "tsv", "pq"].iter().any(|e| e == ext) {
                ...
                io::write_table(&mut df, &output).unwrap_or_else(|_| {
                    eprintln!(
                        "[pineapple::neural::boxes] ERROR: Failed to write embeddings to a table."
                    );
                    std::process::exit(1);
                });
            } else if ext == "npy" {
                ...
                .unwrap_or_else(|_| {
                    eprintln!(
                        "[pineapple::neural::boxes] ERROR: Failed to write embeddings to a npy array."
                    );
                    std::process::exit(1);
                });
            } else if ext == "npz" {
                io::write_embeddings_npz(name, item, spot, data, &output).unwrap_or_else(|_| {
                    eprintln!(
                        "[pineapple::neural::boxes] ERROR: Failed to write embeddings to an npz array."
                    );
                    std::process::exit(1);
                });
            }
