...
pub fn profile_image_mask(args: &ProfileMaskArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::profile::mask] Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    {
        eprintln!(
            "[pineapple::profile::mask] Invalid mode. Argument mode must only contain one or more of: c, f, b, m, p, x."
        );
        std::process::exit(1);
    }

    if min_size < 1 {
        eprintln!("[pineapple::profile::mask] ERROR: min_size cannot be less than 1.0.");
        std::process::exit(1);
    }
    ...
    if image_path == masks_path && args.image_substring == args.mask_substring {
        eprintln!(
            "[pineapple::profile::mask] ERROR: If images and masks are located in same path, different image and mask substrings must be provided."
        );
        std::process::exit(1);
    }
    ...
    if image_files.is_empty() {
        eprintln!(
            "[pineapple::profile::mask] ERROR: No image files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if mask_files.is_empty() {
        eprintln!(
            "[pineapple::profile::mask] ERROR: No mask files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if let Some(ext) = extension {
        if !["csv", "txt", "tsv", "pq"].iter().any(|e| e == &ext) {
            eprintln!(
                "[pineapple::profile::mask] ERROR: Invalid file extension. Must end with one of .csv, .txt, .tsv, .pq."
            );
            std::process::exit(1);
        }
        ...
        if let Some(parent) = output.parent()
            ...
            eprintln!(
                "[pineapple::profile::mask] ERROR: Invalid file path. Parent directory of output file path does not exist."
            );
            std::process::exit(1);
        }
    } else {
        output = ut::path::create_directory(&output).unwrap_or_else(|_| {
            eprintln!("[pineapple::profile::mask] ERROR: Could not create directory.");
            std::process::exit(1);
        });
    }
    ...
    if !success.is_empty() {
        ...
        io::write_table(&mut df, descriptors_path).unwrap_or_else(|_| {
            eprintln!("[pineapple::profile::mask] ERROR: Failed to write descriptors table.");
            std::process::exit(1);
        });
    }
