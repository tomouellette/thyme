...
pub fn profile_image_boxes(args: &ProfileBoxesArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::profile::boxes] Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    if mode.chars().any(|c| !matches!(c, 'c' | 'x')) {
        eprintln!(
            "[pineapple::profile::boxes] Invalid mode. Argument mode must only contain one or more of: c, x."
        );
        std::process::exit(1);
    }

    if min_size < 1 {
        eprintln!("[pineapple::profile::boxes] ERROR: min_size cannot be less than 1.0.");
        std::process::exit(1);
    }
    ...
    if image_path == boxes_path && args.image_substring == args.box_substring {
        eprintln!(
            "[pineapple::profile::boxes] ERROR: If images and boxes are located in same path, different image and bounding box substrings must be provided."
        );
        std::process::exit(1);
    }
    ...
    if image_files.is_empty() {
        eprintln!(
            "[pineapple::profile::boxes] ERROR: No image files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if boxes_files.is_empty() {
        eprintln!(
            "[pineapple::profile::boxes] ERROR: No bounding box files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if let Some(ext) = extension {
        if !["csv", "txt", "tsv", "pq"].iter().any(|e| e == &ext) {
            eprintln!(
                "[pineapple::profile::boxes] ERROR: Invalid file extension. Must end with one of .csv, .txt, .tsv, .pq."
            );
            std::process::exit(1);
        }
        ...
        if let Some(parent) = output.parent()
            ...
            eprintln!(
                "[pineapple::profile::boxes] ERROR: Invalid file path. Parent directory of output file path does not exist."
            );
            std::process::exit(1);
        }
    } else {
        output = ut::path::create_directory(&output).unwrap_or_else(|_| {
            eprintln!("[pineapple::profile::boxes] ERROR: Could not create directory.");
            std::process::exit(1);
        });
    }
    ...
    if !success.is_empty() {
        ...
        io::write_table(&mut df, descriptors_path).unwrap_or_else(|_| {
            eprintln!("[pineapple::profile::boxes] ERROR: Failed to write descriptors table.");
            std::process::exit(1);
        });
    }
