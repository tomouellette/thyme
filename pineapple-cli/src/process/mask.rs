...
pub fn process_image_mask(args: &ProcessMaskArgs) {
    ...
    let threads = if let Some(t) = args.threads {
        ...
        std::thread::available_parallelism().unwrap_or_else(|_| {
            eprintln!("[pineapple::process::mask] Could not automatically assign number of tasks. Please manually set the --threads (-t) argument.");
            std::process::exit(1);
        }).get()
    };
    ...
    {
        eprintln!(
            "[pineapple::process::mask] Invalid mode. Argument mode must only contain one or more of: c, f, b, m, p, x."
        );
        std::process::exit(1);
    }

    if min_size < 1 {
        eprintln!("[pineapple::process::mask] ERROR: min_size cannot be less than 1.0.");
        std::process::exit(1);
    }

    if !constant::SUPPORTED_IMAGE_FORMATS.contains(&image_format.as_str()) {
        eprintln!(
            "[pineapple::process::mask] ERROR: Invalid image_format {}. Must be one of: {:?}.",
            image_format,
    ...
    if !constant::SUPPORTED_ARRAY_FORMATS.contains(&array_format.as_str()) {
        eprintln!(
            "[pineapple::process::mask] ERROR: Invalid array_format {}. Must be one of: {:?}.",
            array_format,
    ...
    if image_path == masks_path && args.image_substring == args.mask_substring {
        eprintln!(
            "[pineapple::process::mask] ERROR: If images and masks are located in same path, different image and mask substrings must be provided."
        );
        std::process::exit(1);
    }
    ...
    if image_files.is_empty() {
        eprintln!(
            "[pineapple::process::mask] ERROR: No image files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if mask_files.is_empty() {
        eprintln!(
            "[pineapple::process::mask] ERROR: No mask files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    let output = ut::path::create_directory(&output).unwrap_or_else(|_| {
        eprintln!("[pineapple::process::mask] ERROR: Could not create directory.");
        std::process::exit(1);
    });
