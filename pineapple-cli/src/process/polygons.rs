...
use pineapple_core::cv::points::draw_centered_points;
...
pub struct ProcessPolygonsArgs {
    ...
    #[arg(
        long,
        short = 'm',
        help = "Mode. One or more of c (complete pixels), f (foreground pixels), b (background pixels), m (binary mask), p (polygons), and x (bounding boxes).",
        default_value = "cm"
    )]
...
pub fn process_image_polygons(args: &ProcessPolygonsArgs) {
    ...
    let threads = if let Some(t) = args.threads {
        ...
        std::thread::available_parallelism().unwrap_or_else(|_| {
            eprintln!("[pineapple::process::polygons] Could not automatically assign number of tasks. Please manually set the --threads (-t) argument.");
            std::process::exit(1);
        }).get()
    };
    ...
    {
        eprintln!(
            "[pineapple::process::polygons] Invalid mode. Argument mode must only contain one or more of: c, f, b, m, p, x."
        );
        std::process::exit(1);
    }

    if min_size < 1 {
        eprintln!("[pineapple::process::polygons] ERROR: min_size cannot be less than 1.0.");
        std::process::exit(1);
    }

    if !constant::SUPPORTED_IMAGE_FORMATS.contains(&image_format.as_str()) {
        eprintln!(
            "[pineapple::process::polygons] ERROR: Invalid image_format {}. Must be one of: {:?}.",
            image_format,
    ...
    if !constant::SUPPORTED_ARRAY_FORMATS.contains(&array_format.as_str()) {
        eprintln!(
            "[pineapple::process::polygons] ERROR: Invalid array_format {}. Must be one of: {:?}.",
            array_format,
    ...
    if image_path == polygons_path && args.image_substring == args.polygon_substring {
        eprintln!(
            "[pineapple::process::polygons] ERROR: If images and polygons are located in same path, different image and polygon substrings must be provided."
        );
        std::process::exit(1);
    }
    ...
    if image_files.is_empty() {
        eprintln!(
            "[pineapple::process::polygons] ERROR: No image files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    if polygon_files.is_empty() {
        eprintln!(
            "[pineapple::process::polygons] ERROR: No polygon files were detected. Please check your path and/or substring identifier."
        );
        std::process::exit(1);
    }
    ...
    let output = ut::path::create_directory(&output).unwrap_or_else(|_| {
        eprintln!("[pineapple::process::polygons] ERROR: Could not create directory.");
        std::process::exit(1);
    });
    ...
    if mode.contains("f") {
        std::fs::create_dir(output.join("foreground")).unwrap();
    }

    if mode.contains("b") {
        std::fs::create_dir(output.join("background")).unwrap();
    }
...
fn extract(
    ...
    for (idx, [min_x, min_y, max_x, max_y]) in bounding_boxes.as_xyxy().iter().enumerate() {
        ...
        let mask_buffer = im::PineappleMask::new(
            w,
            h,
            1,
        ...
        let mask_object = im::PineappleMaskView::new(0, 0, w, h, &mask_buffer);
...
pub async fn run_all(
    ...
    threads: usize,
    verbose: bool,
) -> Vec<(String, Result<u32, PineappleError>)> {
