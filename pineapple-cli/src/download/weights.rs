...
pub fn download_weights(args: &DownloadWeightsArgs) {
    ...
    if args.name.is_none() {
        eprintln!(
            "[pineapple::download::weights] The weights --name/-n must be specified. Run `pineapple download weights --list` to see all available weights."
        );
        std::process::exit(1);
    }
...
fn print_weights() {
    println!("{:^69}", "\n");
    println!("| {:-^74} |", "");
    println!("| {:^74} |", "pineapple".truecolor(103, 194, 69).bold());
