...
use pineapple_data::data::BenchmarkDatasets;
...
pub struct DownloadBenchmarkArgs {
    #[arg(
        short,
        long,
        help = "Dataset name. Run pineapple download benchmark --list` to see all available benchmarks. Run `pineapple download benchmark --all` to download all benchmark datasets"
    )]
...
pub fn download_benchmark(args: &DownloadBenchmarkArgs) {
    ...
    if args.all {
        if args.output.is_none() {
            eprintln!(
                "[pineapple::download::benchmark] Please specify the output path (--output/-o) to save all downloaded benchmark datasets."
            );
            std::process::exit(1);
        }
    ...
    if !(args.name.is_some() && args.output.is_some()) {
        eprintln!("[pineapple::download::benchmark] Both --name/-n and --output/-o must be specified.");
        std::process::exit(1);
    }
...
fn print_benchmark() {
    println!("{:^71}", "\n");
    println!("| {:-^71} |", "");
    println!("| {:^71} |", "pineapple".truecolor(103, 194, 69).bold());
