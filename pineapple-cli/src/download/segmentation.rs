...
use pineapple_core::ut::track::progress_log;
use pineapple_data::data::SegmentationDatasets;
...
pub struct DownloadSegmentationArgs {
    #[arg(
        short,
        long,
        help = "Dataset name. Run pineapple download segmentation --list` to see all available datasets. Run `pineapple download segementation --all` to download all segmentation datasets"
    )]
...
pub fn download_segmentation(args: &DownloadSegmentationArgs) {
    ...
    if args.all {
        if args.output.is_none() {
            eprintln!(
                "[pineapple::download::segmentation] Please specify the output path (--output/-o) to save all downloaded segmentation datasets."
            );
            std::process::exit(1);
        }
    ...
    if !(args.name.is_some() && args.output.is_some()) {
        eprintln!(
            "[pineapple::download::segmentation] Both --name/-n and --output/-o must be specified."
        );
        std::process::exit(1);
    }
...
fn print_segmentation() {
    println!("{:^72}", "\n");
    println!("| {:-^72} |", "");
    println!("| {:^72} |", "pineapple".truecolor(103, 194, 69).bold());
