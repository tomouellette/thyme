...
pub fn measure_moments(args: &MomentsArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::measure::moments] ERROR: Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    let is_image_dir = if let Some(ext) = &image_extension {
        if !constant::SUPPORTED_IMAGE_FORMATS.contains(&ext.as_str()) {
            eprintln!(
                "[pineapple::measure::moments] ERROR: Invalid image extension {}. Must be one of: {:?}.",
                ext,
    ...
    if let Some(output) = args.output.to_owned() {
        if !is_image_dir {
            eprintln!(
                "[pineapple::measure::moments] ERROR: If output is provided, then input image path must specify an image directory."
            );
            std::process::exit(1);
        }
        ...
        if let Some(ext) = &extension {
            if !["csv", "txt", "tsv", "pq"].iter().any(|e| e == ext) {
                eprintln!(
                    "[pineapple::measure::moments] ERROR: Invalid file extension. Must end with one of .csv, .txt, .tsv, .pq."
                );
                std::process::exit(1);
            }
        } else {
            eprintln!(
                "[pineapple::measure::moments] ERROR: Invalid output path. Output file must be a file with a valid extension."
            );
            std::process::exit(1);
        }
        ...
        if image_files.is_empty() {
            eprintln!(
                "[pineapple::measure::moments] ERROR: No image files were detected. Please check your path and/or substring identifier."
            );
            std::process::exit(1);
        }
        ...
        let data: Mutex<Vec<[f32; 24]>> = Mutex::new(Vec::with_capacity(24 * image_files.len()));
        ...
        if is_image_dir {
            eprintln!(
                "[pineapple::measure::moments] ERROR: If output is not provided, then input image path should specify a single file."
            );
            std::process::exit(1);
        }
        ...
        if !image_path.is_file() {
            eprintln!(
                "[pineapple::measure::moments] ERROR: The provided image file path does not exist."
            );
            std::process::exit(1);
        }

        let data = moments(Path::new(&image_path)).unwrap_or_else(|_| {
            eprintln!("[pineapple::measure::moments] ERROR: Failed to measure moments descriptors.");
            std::process::exit(1);
        });
...
fn moments(image_path: &Path) -> Result<[f32; 24], PineappleError> {
...
fn write_moments(data: &[[f32; 24]], name: &Vec<String>, output: &Path) {
    let columns = constant::MOMENTS_DESCRIPTOR_NAMES;
    ...
    io::write_table(&mut df, descriptors_path).unwrap_or_else(|_| {
        eprintln!("[pineapple::measure::moments] ERROR: Failed to write descriptors table.");
        std::process::exit(1);
    });
}
