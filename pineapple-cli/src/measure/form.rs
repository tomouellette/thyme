...
pub fn measure_form(args: &FormArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::measure::form] ERROR: Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    let is_polygon_dir = if let Some(ext) = &polygon_extension {
        if !constant::SUPPORTED_ARRAY_FORMATS.contains(&ext.as_str()) {
            eprintln!(
                "[pineapple::measure::form] ERROR: Invalid polygon extension {}. Must be one of: {:?}.",
                ext,
    ...
    if let Some(output) = args.output.to_owned() {
        if !is_polygon_dir {
            eprintln!(
                "[pineapple::measure::form] ERROR: If output is provided, then input polygons path must specify a polygons directory."
            );
            std::process::exit(1);
        }
        ...
        if let Some(ext) = &extension {
            if !["csv", "txt", "tsv", "pq"].iter().any(|e| e == ext) {
                eprintln!(
                    "[pineapple::measure::form] ERROR: Invalid file extension. Must end with one of .csv, .txt, .tsv, .pq."
                );
                std::process::exit(1);
            }
        } else {
            eprintln!(
                "[pineapple::measure::form] ERROR: Invalid output path. Output file must be a file with a valid extension."
            );
            std::process::exit(1);
        }
        ...
        if let Some(parent) = output.parent()
            ...
            eprintln!(
                "[pineapple::measure::form] ERROR: Invalid file path. Parent directory of output file path does not exist."
            );
            std::process::exit(1);
        }
        ...
        if polygon_files.is_empty() {
            eprintln!(
                "[pineapple::measure::form] ERROR: No polygon files were detected. Please check your path and/or substring identifier."
            );
            std::process::exit(1);
        }
        ...
        if is_polygon_dir {
            eprintln!(
                "[pineapple::measure::form] ERROR: If output is not provided, then input polygon path should specify a single file."
            );
            std::process::exit(1);
        }
        ...
        if !polygons_path.is_file() {
            eprintln!(
                "[pineapple::measure::form] ERROR: The provided polygon file path does not exist."
            );
            std::process::exit(1);
        }

        let data = form(Path::new(&polygons_path)).unwrap_or_else(|_| {
            eprintln!("[pineapple::measure::form] ERROR: Failed to measure form descriptors.");
            std::process::exit(1);
        });
...
fn form(polygons_path: &Path) -> Result<Vec<[f32; 23]>, PineappleError> {
...
fn write_form(data: &[[f32; 23]], name: &Vec<String>, item: &Vec<u32>, output: &Path) {
    ...
    io::write_table(&mut df, descriptors_path).unwrap_or_else(|_| {
        eprintln!("[pineapple::measure::form] ERROR: Failed to write descriptors table.");
        std::process::exit(1);
    });
}
