...
pub fn measure_texture(args: &TextureArgs) {
    if let Some(threads) = args.threads.to_owned() {
        if threads < 1 {
            println!(
                "[pineapple::measure::texture] ERROR: Threads must be set to a positive integer if provided."
            );
            std::process::exit(1);
        }
    ...
    let is_image_dir = if let Some(ext) = &image_extension {
        if !constant::SUPPORTED_IMAGE_FORMATS.contains(&ext.as_str()) {
            eprintln!(
                "[pineapple::measure::texture] ERROR: Invalid image extension {}. Must be one of: {:?}.",
                ext,
    ...
    if let Some(output) = args.output.to_owned() {
        if !is_image_dir {
            eprintln!(
                "[pineapple::measure::texture] ERROR: If output is provided, then input image path must specify an image directory."
            );
            std::process::exit(1);
        }
        ...
        if let Some(ext) = &extension {
            if !["csv", "txt", "tsv", "pq"].iter().any(|e| e == ext) {
                eprintln!(
                    "[pineapple::measure::texture] ERROR: Invalid file extension. Must end with one of .csv, .txt, .tsv, .pq."
                );
                std::process::exit(1);
            }
        } else {
            eprintln!(
                "[pineapple::measure::texture] ERROR: Invalid output path. Output file must be a file with a valid extension."
            );
            std::process::exit(1);
        }
        ...
        if let Some(parent) = output.parent() {
            if !parent.is_dir() && parent.to_str().unwrap() != "" {
                eprintln!(
                    "[pineapple::measure::texture] ERROR: Invalid file path. Parent directory of output file path does not exist."
                );
                std::process::exit(1);
            }
        }
        ...
        if image_files.is_empty() {
            eprintln!(
                "[pineapple::measure::texture] ERROR: No image files were detected. Please check your path and/or substring identifier."
            );
            std::process::exit(1);
        }
        ...
        if is_image_dir {
            eprintln!(
                "[pineapple::measure::texture] ERROR: If output is not provided, then input image path should specify a single file."
            );
            std::process::exit(1);
        }
        ...
        if !image_path.is_file() {
            eprintln!(
                "[pineapple::measure::texture] ERROR: The provided image file path does not exist."
            );
            std::process::exit(1);
        }

        let data = texture(Path::new(&image_path)).unwrap_or_else(|_| {
            eprintln!("[pineapple::measure::texture] ERROR: Failed to measure texture descriptors.");
            std::process::exit(1);
        });
...
fn texture(image_path: &Path) -> Result<[f32; 13], PineappleError> {
...
fn write_texture(data: &[[f32; 13]], name: &Vec<String>, output: &Path) {
    ...
    io::write_table(&mut df, descriptors_path).unwrap_or_else(|_| {
        eprintln!("[pineapple::measure::texture] ERROR: Failed to write descriptors table.");
        std::process::exit(1);
    });
}
