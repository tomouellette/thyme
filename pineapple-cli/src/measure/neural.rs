...
use pineapple_core::constant;
use pineapple_core::error::PineappleError;
use pineapple_core::im;
use pineapple_core::io;
use pineapple_core::ut;
use pineapple_data::data::Weights;
use pineapple_neural::nn::Models;
...
pub fn measure_neural(args: &NeuralArgs) {
    ...
    if !["cpu", "metal", "cuda"].iter().any(|d| d == &device) {
        eprintln!(
            "[pineapple::measure::neural] ERROR: Invalid device. Must be one of: cpu, metal, cuda."
        );
        std::process::exit(1);
    }
    ...
    if device == "cuda" && !cuda_is_available() {
        println!(
            "[pineapple::measure::neural] Device 'cuda' specified but no cuda device was detected."
        );
        std::process::exit(1);
    }
    ...
    if device == "metal" && !metal_is_available() {
        println!(
            "[pineapple::measure::neural] Device 'metal' specified but no metal device was detected."
        );
        std::process::exit(1);
    }
    ...
    let is_image_dir = if let Some(ext) = &image_extension {
        if !constant::SUPPORTED_IMAGE_FORMATS.contains(&ext.as_str()) {
            eprintln!(
                "[pineapple::measure::neural] ERROR: Invalid image extension {}. Must be one of: {:?}.",
                ext,
    ...
    if let Some(output) = args.output.to_owned() {
        if let Some(threads) = args.threads.to_owned() {
            if threads < 1 {
                println!(
                    "[pineapple::measure::neural] Threads must be set to a positive integer if provided."
                );
                std::process::exit(1);
            }
        ...
        if !is_image_dir {
            eprintln!(
                "[pineapple::measure::neural] ERROR: If output is provided, then input image path must specify an image directory."
            );
            std::process::exit(1);
        }
        ...
        if let Some(ext) = &extension {
            ...
            {
                eprintln!(
                    "[pineapple::measure::neural] ERROR: Invalid file extension. Must end with one of .npy, .npz, .csv, .txt, .tsv, .pq."
                );
                std::process::exit(1);
            }
        } else {
            eprintln!(
                "[pineapple::measure::neural] ERROR: Invalid output path. Output file must be a file with a valid extension."
            );
            std::process::exit(1);
        }
        ...
        if let Some(parent) = output.parent()
            ...
            eprintln!(
                "[pineapple::measure::neural] ERROR: Invalid file path. Parent directory of output file path does not exist."
            );
            std::process::exit(1);
        }
        ...
        if image_files.is_empty() {
            eprintln!(
                "[pineapple::measure::neural] ERROR: No image files were detected. Please check your path and/or substring identifier."
            );
            std::process::exit(1);
        }
        ...
        if is_image_dir {
            eprintln!(
                "[pineapple::measure::neural] ERROR: If output is not provided, then input image path should specify a single file."
            );
            std::process::exit(1);
        }
        ...
        if !image_path.is_file() {
            eprintln!(
                "[pineapple::measure::neural] ERROR: The provided image file path does not exist."
            );
            std::process::exit(1);
        }
        ...
        let data = neural(Path::new(&image_path), &model, &device).unwrap_or_else(|_| {
            eprintln!("[pineapple::measure::neural] ERROR: Failed to measure neural descriptors.");
            std::process::exit(1);
        });
...
fn neural(image_path: &Path, model: &Models, device: &Device) -> Result<Vec<f32>, PineappleError> {
    let image = im::PineappleImage::open(image_path)?;
...
fn write_neural(data: &[Vec<f32>], name: &Vec<String>, output: &PathBuf, extension: &str) {
    ...
    if ["csv", "txt", "tsv", "pq"].iter().any(|e| e == &extension) {
        ...
        io::write_table(&mut df, output).unwrap_or_else(|_| {
            eprintln!("[pineapple::measure::neural] ERROR: Failed to write embeddings to a table.");
            std::process::exit(1);
        });
        ...
        .unwrap_or_else(|_| {
            eprintln!("[pineapple::measure::neural] ERROR: Failed to write embeddings to a npy array.");
            std::process::exit(1);
        });
        ...
        io::write_embeddings_npz(name.to_vec(), vec![], vec![], data.to_vec(), &output)
            .unwrap_or_else(|_| {
                eprintln!(
                    "[pineapple::measure::neural] ERROR: Failed to write embeddings to an npz array."
                );
                std::process::exit(1);
            });
    }
