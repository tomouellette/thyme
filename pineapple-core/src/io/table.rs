...
pub fn write_table_csv<P: AsRef<Path>>(
    ...
    let mut output: File = File::create(&path).map_err(|_| {
        PineappleError::OtherError(format!(
    ...
    CsvWriter::new(&mut output)
        ...
        .finish(df)
        .map_err(|_| PineappleError::OtherError("Failed to write CSV file.".to_string()))
}
...
/// ```no_run
/// use polars::prelude::*;
/// use pineapple_core::io::write_table_tsv;
///
/// let column = vec![Column::new("area".into(), [2.5, 3.1, 3.4])];
/// let mut df: DataFrame = DataFrame::new(column).unwrap();
///
...
pub fn write_table_tsv<P: AsRef<Path>>(
    ...
    CsvWriter::new(&mut output)
        ...
        .finish(df)
        .map_err(|_| PineappleError::OtherError("Failed to write TSV file.".to_string()))
}
...
/// ```no_run
/// use polars::prelude::*;
/// use pineapple_core::io::write_table_pq;
///
/// let column = vec![Column::new("area".into(), [2.5, 3.1, 3.4])];
/// let mut df: DataFrame = DataFrame::new(column).unwrap();
///
...
/// ```
pub fn write_table_pq<P: AsRef<Path>>(df: &mut DataFrame, path: P) -> Result<(), PineappleError> {
    let mut output: File = File::create(&path).map_err(|_| {
        PineappleError::OtherError(format!(
            "Failed to create TSV file: {}",
            path.as_ref().to_str().unwrap()
    ...
    ParquetWriter::new(&mut output)
        .finish(df)
        .map(|_| ())
        .map_err(|_| PineappleError::OtherError("Failed to write parquet file.".to_string()))
}
...
/// ```
pub fn write_table<P: AsRef<Path>>(df: &mut DataFrame, path: P) -> Result<(), PineappleError> {
    let extension = path
        .as_ref()
        .extension()
        .and_then(|s| s.to_str())
    ...
    if let Some(ext) = extension {
        match ext.as_str() {
            ...
            _ => Err(PineappleError::OtherError("Failed to write table.".to_string())),
        }
    } else {
