...
impl Polygons {
    ...
    /// let polygons = Polygons::new(data);
    /// ```
    pub fn new(data: Vec<Vec<[f32; 2]>>) -> Result<Self, PineappleError> {
        let n = data.len();
        ...
        if data.len() != n {
            return Err(PineappleError::PolygonsSizeError);
        }
...
impl Polygons {
    ...
    /// ```
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Polygons, PineappleError> {
        let extension = path
            .as_ref()
            .extension()
            .and_then(|s| s.to_str())
...
pub fn read_polygons_json<P: AsRef<Path>>(path: P) -> Result<Polygons, PineappleError> {
    let mut contents = String::new();

    File::open(path)
        .map_err(|err| PineappleError::NoFileError(err.to_string()))?
        .read_to_string(&mut contents)
        .map_err(|err| PineappleError::NoFileError(err.to_string()))?;

    let data: Value = serde_json::from_str(&contents).map_err(|_| PineappleError::PolygonsReadError)?;

    fn to_f32(value: &Value) -> Result<f32, PineappleError> {
...
pub fn write_polygons_json<P, T>(path: P, polygons: &[Vec<[T; 2]>]) -> Result<(), PineappleError>
where
    P: AsRef<Path>,
    T: Serialize,
{
    let file = File::create(path).map_err(|_| PineappleError::PolygonsWriteError)?;
    let writer = BufWriter::new(file);

    serde_json::to_writer(writer, &serde_json::json!({ "polygons": polygons }))
        .map_err(|_| PineappleError::PolygonsWriteError)?;
