...
use crate::im::{MaskingStyle, PineappleMaskView, PineappleViewBuffer};
    ...
    Container: Deref<Target = [T]>,
{
    ...
    pub fn new(
        ...
        if width * height * channels == buffer.len() as u32 {
            ...
            Err(PineappleError::BufferSizeError)
        }
    }
}
    ...
    Container: Deref<Target = [T]>,
{
        ...
        Container: Deref<Target = [T]>,
    {
        if channel >= self.channels() {
            return Err(PineappleError::ChannelBoundsError);
        }
    ...
    T: Clone + ToPrimitive + FromPrimitive,
{
    ...
    pub fn crop(
        ...
        if x + w > self.w || y + h > self.h {
            return Err(PineappleError::ImageError("Cropping coordinates out of bounds"));
        }
...
mod test {
    ...
    fn test_buffer_new_success() {
        let buffer = PineappleBuffer::new(1, 3, 2, [1, 2, 3, 4, 5, 6].as_slice());
        assert!(buffer.is_ok());
    }

    #[test]
    fn test_buffer_new_error() {
        let buffer = PineappleBuffer::new(2, 3, 2, [1, 2, 3, 4, 5, 6].as_slice());
    ...
    fn test_buffer_to_u8() {
        let buffer = PineappleBuffer::new(1, 2, 2, [2.5, 3.9, 4.8, 2.2].as_slice()).unwrap();
    ...
    #[test]
    fn test_iter() {
        let buffer = PineappleBuffer::new(1, 3, 2, [1, 2, 3, 4, 5, 6].as_slice()).unwrap();
    ...
    fn test_iter_channel() {
        ...
        let buffer = PineappleBuffer::new(2, 1, 3, [1, 2, 3, 4, 5, 6].as_slice()).unwrap();
    ...
    #[test]
    fn test_iter_pixels() {
        let buffer = PineappleBuffer::new(1, 4, 2, [1, 2, 3, 4, 5, 6, 7, 8].as_slice()).unwrap();
