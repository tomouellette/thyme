...
impl BoundingBoxes {
    ...
    pub fn new(data: Vec<[f32; 4]>) -> Result<Self, PineappleError> {
        ...
        if data.len() != n {
            return Err(PineappleError::BoxesSizeError);
        }
...
pub fn read_boxes_json<P: AsRef<Path>>(path: P) -> Result<BoundingBoxes, PineappleError> {
    ...
    let data: Value = serde_json::from_str(&contents).map_err(|_| PineappleError::BoxesReadError)?;
    ...
    for key in &BOUNDING_BOX_JSON_VALID_KEYS {
        if let Some(boxes) = data.get(key).and_then(|v| v.as_array()) {
            let boxes: Result<Vec<[f32; 4]>, _> = boxes
                ...
                .map(|item| {
                    item.as_array()
                        .ok_or(PineappleError::BoxesReadError)
                        .and_then(|b| {
                            if b.len() == 4 {
...
pub fn write_boxes_json<P, T>(path: P, boxes: &Vec<[T; 4]>) -> Result<(), PineappleError>
where
    P: AsRef<Path>,
    T: Serialize,
{
    let file = File::create(path).map_err(|_| PineappleError::BoxesWriteError)?;
    let writer = BufWriter::new(file);

    serde_json::to_writer(writer, &serde_json::json!({ "bounding_boxes": boxes }))
        .map_err(|_| PineappleError::BoxesWriteError)?;
